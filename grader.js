...
if(require.main == module) {
	...
	.option('-u, --url <url_to_read>', 'url to check')
        .parse(process.argv);

    var onRead = function(content) {
    ...
    if (program.file) {
        fs.readFile(program.file, function(err, data) {
            if (err) {
                console.log("failed in loading file: " + program.file);
                return;
            }
            onRead(data);
        });
        ...
        rest.get(url_to_read).on('complete', onRead);
    }
} else {
